...
*You do not need to write SQL queries. You do not need to build APIs. This task is only about database design.*
...
- **Infrastructure Understanding (15 marks):** Did the student understand that buildings contain floors and elevators separately? Did they separate elevator configuration from ride activity data?
- **Entity Quality (20 marks):** Did they correctly model building, floor, elevator, request, ride assignment and maintenance entities? Did they avoid merging requests directly into elevators?
- **Relationship Modeling (20 marks):** Are building–floor relationships modeled correctly? Are building–elevator relationships modeled correctly? Is request-to-elevator assignment modeled properly? Are many-to-many relations handled where required?
- **Attribute and Table Design (15 marks):** Are static structures separated from dynamic ride logs? Is elevator status handled separately from ride history? Are maintenance details stored properly instead of mixing inside elevator tables?
- **PK/FK and Junction Tables (15 marks):** Did the student correctly connect requests with elevators? Did they properly model floor–elevator servicing relationships? Are ride assignment tables designed correctly?
- **Real-World Practicality (10 marks):** Does this feel like software that could run inside a real high-rise infrastructure monitoring system?
- **Diagram Neatness (5 marks):** Is the ER diagram clean and readable? Are relationships clearly labeled?
